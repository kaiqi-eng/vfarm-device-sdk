...
def test_async_enqueue_set_state_builds_typed_payload_and_merges_extra() -> None:
    sdk = _AsyncCommandHarness()
    response = _run(
        sdk.enqueue_set_state(
            "device-1",
            target="relay-1",
            state="on",
...
def test_async_enqueue_set_value_builds_typed_payload_and_merges_extra() -> None:
    sdk = _AsyncCommandHarness()
    response = _run(
        sdk.enqueue_set_value(
            "device-2",
            target="fan-speed",
            value=42.5,
            unit="percent",
...
def test_async_enqueue_custom_builds_typed_payload_and_merges_extra() -> None:
    sdk = _AsyncCommandHarness()
    response = _run(
        sdk.enqueue_custom(
            "device-3",
