...
class AsyncAlertsApiMixin:
    ...
    async def create_alert_channel(self, payload: AlertChannelCreate) -> AlertChannelResponse:
        data = await self._request("POST", "/api/v1/alerts/channels", json=payload.model_dump(mode="json", exclude_none=True))
        return AlertChannelResponse.model_validate(data)
    ...
    async def create_alert_rule(self, payload: AlertRuleCreate) -> AlertRuleResponse:
        data = await self._request("POST", "/api/v1/alerts/rules", json=payload.model_dump(mode="json", exclude_none=True))
        return AlertRuleResponse.model_validate(data)
